    ...
    <div>
      <div className="severance-scenario-cards">
        {allResults.map((s) => (
          ...
          >
            <span className="severance-scenario-name">{tx(s.label)}</span>
            <span className={`severance-scenario-total${privacyMode ? ' privacy-blur' : ''}`}>{formatCurrency(s.result.total)}</span>
            <span className="severance-scenario-hint">{s.key === scenario ? tx('Detalhado abaixo') : tx('Ver detalhes')}</span>
          </button>
        ))}
      </div>

      <p className="text-secondary" style={{ fontSize: 11.5, margin: '14px 0' }}>
        {tx(active.description)}
      </p>
      ...
      <div className="severance-total-row">
        <span>{tx('Custo total estimado —')} {tx(active.label)}</span>
      ...
      <p className="text-tertiary" style={{ fontSize: 10.5, marginTop: 10, lineHeight: 1.5 }}>
        {tx('Simulação para')} {formatNumber(active.result.tenureYears, 1)} {tx('anos de casa, considerando desligamento na data de hoje. Estimativa de planejamento (RH/Financeiro) — não substitui o cálculo oficial da folha de pagamento, que usa o saldo real de FGTS com juros e outras variáveis contratuais.')}
      </p>
    </div>
  );
}
