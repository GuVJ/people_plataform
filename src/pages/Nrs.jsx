  ...
  return (
    <div className="page fade-in">
      <div className="page-header">
        <div>
          <h1>{tx('Treinamentos de NRs')}</h1>
          <p className="page-subtitle">{tx('Cobertura por Norma Regulamentadora, reciclagens e vencimentos')}</p>
        </div>
        <ExportButton filename="treinamentos_nrs" sheetName="NRs" rows={exportRows} />
      </div>

      <div className="grid grid-cols-4" style={{ marginBottom: 16 }}>
        <SectionCard title={tx('Cobertura média')}>
          <div className="stat-big">{formatPercent(k.coberturaMedia)}</div>
          <p className="text-secondary" style={{ fontSize: 12 }}>{tx('Da força exposta a risco')}</p>
        </SectionCard>
        <SectionCard title={tx('Treinados no mês')}>
          <div className="stat-big">{formatNumber(k.treinadosMes)}</div>
        </SectionCard>
        <SectionCard title={tx('Treinamentos vencidos')}>
          <div className="stat-big" style={{ color: 'var(--color-danger)' }}>{formatNumber(k.vencidos)}</div>
          <p className="text-secondary" style={{ fontSize: 12 }}>{tx('Reciclagem em atraso')}</p>
        </SectionCard>
        <SectionCard title={tx('Vencendo (60 dias)')}>
          <div className="stat-big">{formatNumber(k.vencendo60d)}</div>
        </SectionCard>
      </div>

      <div style={{ marginBottom: 16 }}>
        <SectionCard title={tx('Cobertura por NR')} subtitle={tx('% da força exposta treinada e em dia')}>
