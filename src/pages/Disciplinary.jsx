    ...
    <div className="page fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Advertências, suspensões e justa causa por tipo, motivo e mês</p>
        </div>
        <ExportButton filename="medidas_disciplinares" sheetName="Disciplinar" rows={exportRows} />
      </div>

      <div className="grid grid-cols-4" style={{ marginBottom: 16 }}>
        <SectionCard title="Total (12 meses)">
          <div className="stat-big">{formatNumber(k.total12)}</div>
        </SectionCard>
        <SectionCard title="Taxa por 100 colaboradores">
          <div className="stat-big">{formatNumber(k.taxaPor100, 1)}</div>
        </SectionCard>
        <SectionCard title="Suspensões">
          <div className="stat-big">{formatNumber(k.suspensoes)}</div>
        </SectionCard>
        <SectionCard title="Reincidência">
          <div className="stat-big">{formatPercent(k.reincidencia)}</div>
          <p className="text-secondary" style={{ fontSize: 12 }}>Colaboradores com 2+ medidas</p>
        </SectionCard>
      </div>

      <div style={{ marginBottom: 16 }}>
        <SectionCard title="Evolução por tipo" subtitle="Últimos 12 meses">
          <StackedBarChart data={d.monthly} series={d.series} height={240} formatValue={(v) => formatNumber(v)} />
        </SectionCard>
      </div>

      <div className="grid grid-cols-2">
        <SectionCard title="Por tipo de medida">
          <DonutChart data={d.byType} centerValue={formatNumber(k.total12)} centerLabel="medidas" formatValue={(v) => formatNumber(v, 1)} />
        </SectionCard>
        <SectionCard title="Principais motivos">
